...
srun -N 4 -p opteron --pty /bin/bash

echo "#Flume variables start
...
flume/bin/flume-ng agent -n TwitterAgent -f flume/conf/flume-twitter.conf -c flume/conf/
hdfs dfs -get /user/tweets
